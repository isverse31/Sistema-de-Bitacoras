...
<head>
    ...
    <style>
        ...
        .header-table img {
            width: center;
            height: auto;
            max-height: 4000px; /* Ajusta la altura máxima */
        }
        .header-left {
            text-align: center;
            font-size: 12px;
...
<body>
    ...
    <?php if (!empty($bitacora)): ?>
        <?php foreach ($bitacora as $index => $detalle): ?>
            ...
            <div class="camera-title">
                Cámara <?php echo $index + 1; ?>: <?php echo isset($detalle['observaciones']) && !empty($detalle['observaciones']) ? $detalle['observaciones'] : 'Sin detalles'; ?>
            </div><br>
    ...
    <p><strong>Comentarios:</strong> <?php echo isset($bitacora[0]['comentario']) ? $bitacora[0]['comentario'] : 'No hay comentarios'; ?></p><br>
